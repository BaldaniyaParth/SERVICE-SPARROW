...
exports.updateServiceCategory = catchAsyncError(async (req, res, next) => {
    try {
        ...
        const { name, description, description1, description2, description3, description4, description5, description6, image } = req.body;
        ...
        // Check if there are files to update image
        ...
        // Upload and store multiple image
        if (image && Array.isArray(image) && image.length > 0) {
            const uploadedImage = await Promise.all(image.map(uploadImage));
            serviceCategory.image = uploadedImage;
        }
